...
def whois_lookup_domain(domain):
    # whois lookup
    try:
...
def rdap_lookup_domain(domain, timeout=10):
    # RDAP lookup
...
def get_company_from_domain(domain):
    ...
    if org:
        ...
        if not skip:
            return org
    # strip labels and retry
    parts = d.split('.')
    while len(parts) > 2:
        parts.pop(0)
        nd = '.'.join(parts)
        org = whois_lookup_domain(nd)
        if org and all(b not in org.lower() for b in BAD_WORDS):
            return org
        org = rdap_lookup_domain(nd)
        if org:
            return org
    # final fallback
    final = whois_lookup_domain('.'.join(parts))
    if final:
        return final
    return "Unknown"

def get_company_from_ip(ip):
...
def classify_company(company, first_party_name):
    # classify company
    if not company:
        return "Third Party"
...
def main():
    ...
    for i, row in df1.iterrows():
        ...
        if "\n" in dom:
            dom = dom.split("\n")[0]
        org = get_company_from_domain(dom)
        party = classify_company(org, fname)
        try:
            ip = socket.getaddrinfo(dom, None)[0][4][0]
            country = geolocate_ip(ip, reader)
        except:
            country = "Unknown"
